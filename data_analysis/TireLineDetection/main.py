...
def get_tire_position(img):
    one_third_way = len(img[0]) // 3
    two_third_way = 2 * one_third_way
    start_y = len(img) - 1

    left_tire_outside = (0, 0)
    right_tire_outside = (0, 0)
    left_tire_inside = (0, 0)
    right_tire_inside = (0, 0)


    for i in range(0, start_y, 1):
        for j in range(0, one_third_way, 1):
            if img[i, j] == 255:
                left_tire_outside = (j, i)
                break
        
        for j in range(len(img[0]) - 1, two_third_way, -1):
            if img[i, j] == 255:
                right_tire_outside = (j, i)
                break

        for j in range(one_third_way, 0, -1):
            if img[i, j] == 255:
                left_tire_inside = (j, i)
                break
        for j in range(two_third_way,  len(img[0]) - 1, 1):
            if img[i, j] == 255:
                right_tire_inside = (j, i)
                break

    return left_tire_outside, right_tire_outside, left_tire_inside, right_tire_inside
...
if __name__ == "__main__":
    with open("TireLineDetection/yolo.yaml", "r") as f:
        classes = yaml.safe_load(f)
    print(classes)
    # yolo = torch.hub.load("ultralytics/yolov5", "yolov5l", pretrained=True)
    yolo = YOLO()
    folders = os.listdir("TireLineDetection/data/images")[26:]
    for folder_num, folder in enumerate(folders):
        ...
        images = [f"TireLineDetection/data/images/{folder}/" + i for i in os.listdir(f"TireLineDetection/data/images/{folder}")]
        # images = images[]
            ...
            for result in results:
                ...
                for j, class_label in enumerate(class_predictions):
                    ...
                    if label == "car" or label == "truck" or label == "bus":
                        ...
                        edges = get_edges(cropped_image)
                        left_tire_outside, right_tire_outside, left_tire_inside, right_tire_inside = get_tire_position(edges)
                        
                        # Find the wheel size, then bump the right_tire_side x by that amount
                        d_outside = right_tire_outside[0] - left_tire_outside[0]
                        d_inside = right_tire_inside[0] - left_tire_inside[0]

                        width_four_wheels = d_outside - d_inside
                        wheel_width = width_four_wheels / 4

                        left_tire = left_tire_inside
                        right_tire = (int(right_tire_inside[0] + wheel_width), right_tire_inside[1])

                        middle_x = (right_tire[0] + left_tire[0]) // 2

                        cv2.rectangle(cropped_image, (middle_x, (left_tire[1] + right_tire[1]) // 2), (middle_x, 0), (255, 0, 0), 2)
                        cv2.putText(cropped_image, f"@{middle_x + x} pixels", (middle_x + 5, 100), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 0), 4)
                        cv2.putText(cropped_image, f"ID: {tracking_ids[j]}", (middle_x + 5, 150), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 0), 4)

                        centers_in_pixels.append((middle_x + x, (left_tire[1] + right_tire[1]) // 2))
                        car_info.append((left_tire + (x, y), right_tire + (x, y), (middle_x + x, ((left_tire[1] + right_tire[1]) // 2) + y), h-y))
