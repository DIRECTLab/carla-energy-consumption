...
def test_get_agents_ev_params():
    agent_classes = get_agents(os.path.join('tests', 'input', 'agents4.csv'))
    try:
        assert agent_classes[0]['ev_params']['A_f'] == 2.22
        assert agent_classes[1]['ev_params']['A_f'] == 2.3316
    except AssertionError:
        traceback.print_exc()
        print(f"{agent_classes=}")
        print()
...
if __name__ == "__main__":
    tests = (
        test_get_agents1, 
        test_get_agents2, 
        test_get_agents_color,
        test_get_agents_ev_params,
    )
    success = 0
    total = 0
    for test in tests:
