...
show_help() {
  ...
  echo ""
  echo "Options:"
  echo "  -a                Runs the automated simulation demo. Cannot be combined with other options."
...
if $AUTOMATED_DEMO; then
  python input/creation/charger_options.py 2 1 -i 0 --power 100_000 --efficiency 0.95 --seed 0 -m Town10HD > input/examples/automation-demo-chargers.csv &&
  python multitracking.py input/examples/tracked_agents.csv output/Town10_lap -w input/examples/automation-demo-chargers.csv --seed 0 -t 230 -d 0.05
else
  if $DEBUG; then
    python manual_control_steeringwheel.py ./input/examples/$VEHICLE.csv -w ./input/examples/$CHARGERS.csv --res $RES;
  else
    python manual_control_steeringwheel.py ./input/examples/$VEHICLE.csv -w ./input/examples/$CHARGERS.csv --res $RES > /dev/null 2>&1 &
  fi
fi
